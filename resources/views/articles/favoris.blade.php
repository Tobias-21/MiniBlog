<x-layouts.app>
    ...
    <x-slot:search>
        <div class="flex justify-center mb-15">
            <form action="{{ route('articles.favoris') }}" method="GET" class="flex">
                @csrf
                @method('GET')
                <input type="text" name="searchs" value="{{ request('searchs') }}" placeholder="Rechercher un article..." class="border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
    ...
    @empty
        ...
        <p class=" text-gray-700  text-center mt-5"> Aucun article favori </p>
    @endforelse
